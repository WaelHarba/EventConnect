	...
	({ options, onValueChange, variant, defaultValue = [], placeholder = "Select options", animation = 0, maxCount = 3, modalPopover = false, asChild = false, className, ...props }, ref) => {
		...
		const toggleAll = () => {
			if (selectedValues.length === options.length) {
				...
				const allValues = options.map((option) => option.email);
							...
							<div className="flex justify-between items-center w-full">
								<div className="flex flex-wrap items-center">
									{selectedValues.slice(0, maxCount).map((value) => {
										const option = options.find((o) => o.email === value);
										const IconComponent = option?.profilePic;
										return (
					...
					<Command>
						...
						<CommandList>
							...
							<CommandGroup>
								...
								{options.map((option) => {
									const isSelected = selectedValues.includes(option.email);
									return (
										<CommandItem key={option.email} onSelect={() => toggleOption(option.email)} className="cursor-pointer">
											<div className={cn("mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary", isSelected ? "bg-primary text-primary-foreground" : "opacity-50 [&_svg]:invisible")}>
												<CheckIcon className="h-4 w-4" />
											</div>
											{option.profilePic && (
												<Avatar className="mr-2 h-4 w-4 text-muted-foreground">
													<AvatarImage src={option.profilePic} />
													<AvatarFallback>Avatar Image Missing</AvatarFallback>
												</Avatar>
											)}
